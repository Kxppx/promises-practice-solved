...
These exercises may be confusing after reading the instructions in [CAUTION](#caution), as the built-in functions appear to be missing important parameters.

files: `e2.js`, `e3.js`, `e5.js`, `e7.js`, `e9.js`

# Important

On file `e11.js`, I ran into a hiccup while testing. In the `db.json` file, the `id` values were normal numbers, but when I ran `json-server` and viewed the data in the browser, the IDs appeared as strings `{ id: "1" }`. This caused the test to fail, even though the requirements were met.

After a couple of restarts, the issue somehow seemed to resolve on its own, but I wanted to flag it just in case it happens to others.
